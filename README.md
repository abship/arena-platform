...
### API Server
- **Dev-only match lifecycle endpoints exist and are env-gated.** `POST /matchmaking/dev/create-match` and `POST /matchmaking/dev/resolve-match` exist only to exercise the minimum-playable flow until automatic queue-fill → match creation ships. They return 404 when `ENABLE_DEV_ENDPOINTS` is disabled in production.
- **Signup hardcodes `User.country='US'` and `region=null`.** Real geolocation-at-signup is deferred; location enforcement currently happens at queue join via `GeoService`.
- **No email verification flow yet.** JWT-only stateless auth is sufficient for Phase 1 minimum-playable scope; email verification is deferred to the WHEN-THEN trigger for real email provider integration.
- **No password reset flow yet.** There is no reset-token or outbound email pipeline yet; add it when the email provider lands.
- **Matchmaking payout calculator wiring covers fixed-pot games plus coinflip only.** Progressive-pool, house-edge, and crash games resolve through different code paths and will need dedicated route/service flows when those gameplay servers ship.
- **CORS allows all origins.** `cors()` with no config is fine for dev/Phase 1 but must be restricted to the actual frontend origin(s) before production deployment.
- **401 responses omit `message` field.** All other error responses include `{ error, message, context? }` but 401 from auth middleware returns only `{ error: 'UNAUTHORIZED' }`. Cosmetic inconsistency; fix when standardizing the API error contract.

### Websocket Package
- **JWT verify logic is duplicated with `servers/api/`.** `servers/websocket/src/lib/verify-token.ts` intentionally mirrors `servers/api/src/lib/auth.ts` because there is no shared `@arena/auth` package yet. Extract when a third consumer appears or when auth logic changes.
- **`join-match` authorization depends on prior `player-joined` broadcasts.** The gateway intentionally tracks match membership from broadcaster events instead of expanding `GameInstanceHost`. This is reliable for the server-initiated flow (match created → player-joined broadcast → client notified → client joins room), but a client that tries `join-match` before those broadcasts arrive can race.
- **Reconnect grace is per gateway instance.** Pending leaves and active socket membership live in memory inside one process. Multi-instance websocket deployment will need shared presence/timer state (Redis or similar).

### Game Server Package
- **No persistence of game state.** Instances are in-memory only — a server crash loses all active matches. Acceptable for Phase 1 through beta; production game-server deployments will need state snapshots or Redis-backed state.
- **Host is single-process.** No horizontal scaling — one process hosts all game instances. Same constraint as the matchmaking in-memory queue; Redis-backed host is future work.
...
### Integration Test
- **Requires Docker running locally.** `npm run test:integration` starts a Postgres 16 container via docker-compose. Without Docker, the command fails immediately. This is expected — install Docker Desktop and ensure it's running.
